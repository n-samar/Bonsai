...
for L in [2, 4, 8, 16, 32, 64, 128]:
    d[L] = []    
    for P in [1, 2, 4, 8, 16, 32]:
        d[L].append(16.0/(N/(P*f)*(math.ceil(math.log(N)/math.log(2*L) - 1) + 1)))
        print "P = " + str(P) + ", L = " + str(L) + ": " + str(16.0/(N/(P*f)*(math.ceil(math.log(N)/math.log(2*L) - 1) + 1))) + " GiB/s"
...
plt.ylabel('End-to-End Throughput (log scale) [GiB/s]')
plt.legend()
plt.show()    
'''
