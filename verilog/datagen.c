...
int main(int argc, char ** argv) {
    ...
    int out_records_per_tuple = (argc>=6) ? atoi(argv[5]) : records_per_tuple;

    int n = ways * records_per_way;
    int i, j;
    printf("datagen %d way, %d records_per_way, %dout_records_per_tuple.\n", ways, records_per_way, out_records_per_tuple);

    uint32_t *buf;
    buf = calloc(n+1, sizeof(uint32_t));
	...
	if (i%out_records_per_tuple == out_records_per_tuple-1) {
	  ...
	  fprintf(ans, "%020x", buf[i+out_records_per_tuple-1-2*(i%out_records_per_tuple)]);	  
	}
      }
      fclose(ans);
    }    
